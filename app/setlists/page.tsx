  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 relative">
      ...
      <div className="max-w-3xl mx-auto p-6">
        <h1 className="text-3xl font-bold text-blue-700 mb-6">🎶 LCOC Praise Team Setlists</h1>

        {/* Create new setlist */}
        <div className="bg-white shadow-lg rounded-lg p-6 mb-8 border border-gray-200">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">Create New Setlist</h2>
          <input type="date" value={date} onChange={e => setDate(e.target.value)}
            className="w-full border rounded-lg px-3 py-2 mb-2 focus:ring-2 focus:ring-blue-400" />
          <input type="text" placeholder="Setlist name" value={name} onChange={e => setName(e.target.value)}
            className="w-full border rounded-lg px-3 py-2 mb-2 focus:ring-2 focus:ring-blue-400" />
          ...
          <div className="max-h-40 overflow-y-auto border rounded-lg p-3 bg-gray-50 mb-4">
            {filteredSongs.map(song => (
              <label key={song.id} className="flex items-center gap-2 py-1">
                <input type="checkbox" checked={selectedSongs.includes(song.id)}
                  onChange={e => {
                    if (e.target.checked) setSelectedSongs(prev => (prev.includes(song.id) ? prev : [...prev, song.id]));
                    else setSelectedSongs(prev => prev.filter(id => id !== song.id));
                  }} />
          ...
          {message && <div className={`mt-3 text-sm px-4 py-2 rounded-lg ${message.type === "success" ? "bg-green-100 text-green-700 border border-green-300" : "bg-red-100 text-red-700 border border-red-300"}`}>{message.text}</div>}
        </div>

        {/* Existing setlists */}
        <div className="space-y-4">
          {setlists.map(setlist => (
            <div key={setlist.id} className="bg-white shadow-md rounded-lg p-5 border border-gray-200 hover:shadow-xl transition">
              <div className="flex justify-between cursor-pointer" onClick={() => setExpandedSetlist(expandedSetlist === setlist.id ? null : setlist.id)}>
                <h2 className="text-lg font-semibold text-blue-600">
                  {setlist.name || "Untitled"} — <span className="text-gray-500">{setlist.date}</span>
                </h2>
                <span className="text-gray-500">{expandedSetlist === setlist.id ? "▲" : "▼"}</span>
              </div>
                      ...
                      <li key={s.song_id} className="mb-2">
                        <div className="flex justify-between items-start gap-2">
                          <div>
                            ...
                            {s.songs.lyrics && <pre className="whitespace-pre-wrap text-sm bg-gray-50 p-2 rounded mt-1 border">{s.songs.lyrics}</pre>}
                          </div>
                          <div className="flex flex-col gap-1">
                            <button disabled={idx === 0} onClick={() => moveSong(setlist.id, idx, "up")}
                              className="bg-gray-300 px-2 rounded hover:bg-gray-400">↑</button>
      ...
      {editingSetlist && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 w-96 shadow-lg">
            <h2 className="text-xl font-bold mb-4">Edit Setlist</h2>
            ...
            <div className="max-h-40 overflow-y-auto border rounded-lg p-3 bg-gray-50 mb-4">
              {filteredEditSongs.map(song => (
                <label key={song.id} className="flex items-center gap-2 py-1">
                  <input type="checkbox" checked={editSelectedSongs.includes(song.id)}
                    onChange={e => {
                      if (e.target.checked) setEditSelectedSongs(prev => (prev.includes(song.id) ? prev : [...prev, song.id]));
                      else setEditSelectedSongs(prev => prev.filter(id => id !== song.id));
                    }} />
                  <span className="text-gray-800">{song.title}</span>
                </label>
              ))}
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setEditingSetlist(null)}
                className="bg-gray-300 px-3 py-1 rounded hover:bg-gray-400 transition">Cancel</button>
              <button onClick={saveEdit}
                className="bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700 transition">Save</button>
            </div>
          </div>
        </div>
      )}
